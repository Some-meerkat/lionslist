...
export default function CommunityPage() {
  ...
  async function fetchData() {
    ...
    (listingsData || []).forEach((l) => {
      if (!listingMap[l.seller_id]) listingMap[l.seller_id] = [];
      listingMap[l.seller_id].push(l);
    });
    ...
    const mktMap = {};
    (mktData || []).forEach((m) => {
      if (!mktMap[m.creator_id]) mktMap[m.creator_id] = [];
      mktMap[m.creator_id].push(m);
    });
    setUserMarketplaces(mktMap);
        ...
        <>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {filtered.map((u) => {
                const uListings = userListings[u.id] || [];
                const activeListings = uListings.filter((l) => !l.sold);
                const soldListings = uListings.filter((l) => l.sold);
                const uMkts = userMarketplaces[u.id] || [];
                const hasActivity = uListings.length > 0 || uMkts.length > 0;
                const isExpanded = expanded[u.id];

                return (
                  <Card key={u.id} className="!p-4">
                    <div
                      className={`flex justify-between items-start ${hasActivity ? "cursor-pointer" : ""}`}
                      onClick={() => hasActivity && toggleExpand(u.id)}
                    >
                      <div>
                    ...
                    <div className="flex gap-2 mt-3 flex-wrap">
                      ...
                      {uMkts.length > 0 && (
                        <Badge color="blue">
                          {uMkts.length} marketplace{uMkts.length !== 1 ? "s" : ""}
                        </Badge>
                      )}
                      {!hasActivity && (
                    ...
                    {isExpanded && (
                      <div className="mt-4 pt-3 border-t border-gray-100 space-y-3">
                        {activeListings.length > 0 && (
                          <div>
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Active Listings</p>
                              ...
                              {activeListings.map((l) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {l.price > 0 && (
                                      <span className="text-sm font-semibold text-green-600 shrink-0 ml-2">${l.price}</span>
                                    )}
                                  </div>
                                );
                              })}
                        ...
                        {uMkts.length > 0 && (
                          <div>
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Marketplaces Created</p>
                            <div className="space-y-1.5">
                              {uMkts.map((m) => {
